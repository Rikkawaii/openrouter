...
public class LlmRouterService {
    ...
    private Mono<ChatCompletionResponse> chatWithFallback(ChatCompletionRequest request, Set<String> excludedIds) {
        ChannelConfig channel = selectBestChannel(request, excludedIds);
        if (channel == null) {
     ...
     */
    private ChannelConfig selectBestChannel(ChatCompletionRequest request, Set<String> excludedIds) {
        List<ChannelConfig> channels = configStore.getChannels();
        if (channels == null || channels.isEmpty()) {
            return null;
        }
        // 排除已经失败过的节点后再进行路由打分
        List<ChannelConfig> candidates = channels.stream()
    ...
    private LlmClientAdapter getAdapterForChannel(ChannelConfig channel) {
     ...
     * 则降级返回第一个模型作为兜底。
     */
    private String selectCompatibleModel(ChatCompletionRequest request, ChannelConfig channel) {
